...
if __name__ == '__main__':
    ...
    parser = OptionParser(
        "usage: %prog -d {directory containing source files} -e {executable name}")
    parser.add_option('-d', '--dir', action='store', type='string',
                      dest='dir', help='Source file directory path')
    parser.add_option('-e', '--exec', action='store', type='string',
                      dest='executable', help='Name of executable generated by Makefile')
    ...
    for f in glob.glob(dir_name+'src/ngircd/*.c.units.mod'):
        ...
        if True:
            ...
            os.system('llvm-link-3.4 -o '+dir_name+'src/ngircd/' +
                      exec_name+' '+dir_name+'src/ngircd/*.o')
            os.system(klee_command + ' --output-dir='+fl_name_noext +
                      '/ '+dir_name+'src/ngircd/'+exec_name+klee_sym_args)
            os.system('mv '+fl_name_noext+'.c ' +
                      fl_name_noext+'.c.units.finished')
